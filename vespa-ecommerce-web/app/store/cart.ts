...
// Debounce untuk update quantity agar tidak spam API saat user klik cepat
...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  fetchCart: async (forceSilent = false) => {
    ...
    try {
      ...
      set((state) => {
         ...
         return { 
             cart: data, 
             isLoading: false, // Pastikan loading mati setelah selesai
             isHydrated: true,
  ...
  updateItemQuantity: (cartItemId, newQuantity) => {
    ...
    // Optimistic update di UI agar responsif
    set(state => {
    ...
    // Kirim ke backend dengan debounce
  ...
  getSummary: () => {
    ...
    const subtotal = selectedCartItems.reduce((acc, item) => {
        // Gunakan harga final yang sudah dihitung backend (termasuk diskon tier/rules)
        // Pastikan properti 'price' pada item.product dari API sudah merupakan harga final
        // atau sesuaikan jika ada field khusus seperti 'finalPrice'
        return acc + (item.product.price * item.quantity);
    }, 0);
    
    // Estimasi pajak di frontend untuk display. Perhitungan asli tetap di backend saat createOrder.
