...
function FilterPopup({
  ...
  const [tempCategoryIds, setTempCategoryIds] = useState(
    new Set(currentFilters.categoryId)
  );
  const [tempBrandIds, setTempBrandIds] = useState(
    new Set(currentFilters.brandId)
  );
  ...
  const filteredCategories = useMemo(() => {
    ...
    return allItems.filter((c) =>
      c.name.toLowerCase().includes(searchTerm.category.toLowerCase())
    );
  ...
  const filteredBrands = useMemo(
    () =>
      allBrands?.filter((b) =>
        b.name.toLowerCase().includes(searchTerm.brand.toLowerCase())
      ) || [],
  ...
  const handleApply = () => {
    onApplyFilters({
      categoryId: Array.from(tempCategoryIds),
      brandId: Array.from(tempBrandIds),
    });
    setIsOpen(false);
  };
  ...
  const activeFilterCount =
    (currentFilters.categoryId?.length || 0) +
    (currentFilters.brandId?.length || 0);
...
export default function ProductClient() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      {/* balik ke container lama, tanpa max-w-6xl */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                  ...
                  {queryParams.search && (
                    <p className="text-sm font-medium text-gray-800">
                      <strong>Pencarian:</strong> "{queryParams.search}"
                    </p>
                  )}
                </div>
              </div>
        ...
        >
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-4 -mx-4 px-4 w-full md:w-auto">
            ...
            {allCategories?.map((category) => (
              <Button
                key={category.id}
                variant={
                  queryParams.categoryId?.includes(category.id) ? 'default' : 'outline'
                }
                size="sm"
                className="rounded-full shrink-0"
          ...
          <div className="flex items-center gap-3 md:gap-4 flex-shrink-0 w-full md:w-auto justify-between md:justify-end">
            ...
            <Select
              ...
              defaultValue={`${queryParams.sortBy}-${queryParams.sortOrder}`}
            >
