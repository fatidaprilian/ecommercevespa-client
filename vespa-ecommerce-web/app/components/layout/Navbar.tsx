...
import {
  ...
  Heart, ChevronDown, Package, Wrench, Zap, Sparkles
} from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="hidden md:flex items-center space-x-6">
              {navigationLinks.map((link) => (
                <div
                  key={link.name}
                  // --- PERBAIKAN 1: Menambahkan padding-bottom untuk memperluas area hover ---
                ...
                >
                  ...
                  {link.hasDropdown && activeDropdown === link.name && (
                    <motion.div
                      ...
                      // --- PERBAIKAN 2: Menghapus `mt-4` untuk menghilangkan celah ---
                    ...
                    >
                      <div className="p-4 grid grid-cols-1 gap-2">
                        {categories.map((category) => (
                          ...
                          >
                            <category.icon className="w-5 h-5 mr-3 text-[#52616B]" />
      ...
      {/* Mobile Menu - Tidak ada perubahan di sini */}
      <AnimatePresence>
        {isOpen && (
           <div
                 ...
                 <div key={link.name}>
                   {link.hasDropdown ? (
                         ...
                         {activeDropdown === link.name && (
                           ...
                           >
                             {categories.map(category => (
                               ...
                               >
                                 <category.icon className="w-5 h-5 mr-3 text-gray-400" />
                                 {category.name}
                               </Link>
                             ))}
                           </motion.div>
                     ...
                     <Link
                       href={link.href}
                       onClick={() => setIsOpen(false)}
                       className={`block text-lg font-semibold p-3 rounded-lg hover:bg-gray-100 ${pathname === link.href ? 'text-blue-600 bg-blue-50' : 'text-gray-700'}`}
                     >
                       {link.name}
                     </Link>
                   )}
