...
function ShipmentTracking({ order }: { order: Order }) {
    ...
    return (
        <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Truck size={20}/> Informasi Pengiriman</CardTitle></CardHeader>
            <CardContent className="space-y-4">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => copyToClipboard(waybillId, 'Nomor resi')}><Copy className="h-4 w-4"/></Button>
                    </div>
                </div>
...
function StatusAndActions({ order, onValidate, onCreateShipment, isStatusLoading, isShipmentLoading }: any) {
    const statusStyle = `px-2 py-1 text-sm font-bold rounded-full ${ order.status === 'PAID' ? 'bg-green-100 text-green-800' : order.status === 'PROCESSING' ? 'bg-orange-100 text-orange-800' : order.status === 'SHIPPED' ? 'bg-blue-100 text-blue-800' : order.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800' }`;
    return (
        <Card>
                ...
                {order.status === 'PENDING' && order.payment?.proofOfPayment && (
                    <div className="border-t pt-4 space-y-4">
                        ...
                        {order.payment.manualPaymentMethod ? (
                            ...
                            <div className="bg-yellow-50 p-3 rounded-md text-sm text-yellow-800"><p>Informasi bank tujuan tidak tercatat.</p></div>
                        )}
            ...
            <CardContent>
                <Table>
                    ...
                    <TableBody>
                        ...
                        <TableRow className="font-extrabold text-lg bg-secondary"><TableCell colSpan={4} className="text-right">Total Pembayaran</TableCell><TableCell className="text-right">{formatPrice(order.totalAmount)}</TableCell></TableRow>
                    </TableBody>
                </Table>
            </CardContent>
        </Card>
