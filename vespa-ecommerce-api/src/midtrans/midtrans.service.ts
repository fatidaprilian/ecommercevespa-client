...
  }
  ...
  /**
   * Memanggil Midtrans Cancel API untuk membatalkan transaksi yang sedang pending.
   * Ini digunakan saat retry agar order_id bisa dipakai lagi.
  ...
  async cancelTransaction(orderId: string): Promise<any> {
    ...
    // Encode serverKey ke Base64 (Buffer adalah global di Node.js)
    const base64Key = Buffer.from(serverKey + ':').toString('base64');

    try {
      this.logger.log(`Attempting to cancel Midtrans transaction: ${orderId}`);
      ...
      if (!response.ok) {
        // Jika status code bukan 2xx (misal 404 jika tidak ditemukan, 401, dll)
        // Kita log sebagai warning tapi tidak melempar error,
        // agar proses retry (pembuatan transaksi baru) tetap bisa lanjut.
        this.logger.warn(`Failed to cancel Midtrans transaction ${orderId}. Status: ${response.status}, Response: ${JSON.stringify(data)}`);
        return data;
      }

      this.logger.log(`Successfully cancelled Midtrans transaction ${orderId}: ${JSON.stringify(data)}`);
      return data;

    } catch (error) {
      this.logger.error(`Error during fetch/cancel for Midtrans transaction ${orderId}:`, error);
      // Jangan lempar error agar proses retry tetap berjalan
      return null;
    }
  }
  // --- AKHIR FUNGSI BARU ---
